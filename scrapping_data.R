...
#    }
    ...
    if(i>27){
       datos<-as.numeric(paste0(unlist(str_extract_all(gsub(unlist(strsplit(str_extract(texto_c, pattern="Globally.*[0-9] confirmed \\([0-9]+ .*\\)NUEVA_LINEA"), split="confirmed \\([0-9]+ .*\\)NUEVA_LINEA"))[1], pattern="2019|11-12|-19|24 hours",replacement=""), pattern="[:digit:]")),collapse=""))
    }
